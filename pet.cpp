...
// Note that the generate function will not be used in game, but as a help function to pet::tame.
// formula of utkdmg = (100*pl)/int(rarity) (more rare, the int(rarity) will be smaller)

// Helper Function
bool pet::generate(double lowerBound, double upperBound, double probability) {
...
void pet::tame(std::string pet, char rarity, std::map<std::string, bool> taming_list){
    bool generation = true;
    if(rarity=='c'){
        generation = pet::generate(0.6, 0.8, 0.6);
    } else if(rarity=='u'){
        generation = pet::generate(0.5, 0.7, 0.3);
    } else if(rarity=='r'){
        generation = pet::generate(0.4, 0.7, 0.2);
    } else if(rarity=='e'){
        generation = pet::generate(0.3, 0.6, 0.1);
    } else if(rarity=='l'){
        generation = pet::generate(0.15, 0.2, 0.1);
    } else if(rarity=='m'){
        generation = pet::generate(0, 0.05, 0.005);
    }
    if(generation==true){
...
void pet::command_pet(std::string pet, int mode, int xp, bool stay, bool follow, bool hunt, bool add_mode){
    if(mode == 1){
        stay = true;
...
void utk(std::string pet, int pet_level, char rarity, int o_health){
    ...
    dmg = (100 * pet_level) / r;
    std::cout << "Your pet " << pet << "is dealing " << dmg << " in his UTK!" << std::endl;
    o_health -= dmg;
}

void feed(std::string pet, std::string food, bool want_to[], bool have_in_invn[], int index, int salv){
    int f = food[0]-'0';
    if(want_to[index]==true && have_in_invn[index]==true){
        salv+=f;
        std::this_thread::sleep_for(std::chrono::seconds(1));
