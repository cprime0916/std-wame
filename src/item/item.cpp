...
void item::sequence(std::vector<std::string>& items, std::vector<bool> is_heal, std::vector<bool> is_harm, std::vector<int> hp, std::vector<int> dmg, bool order, bool item_mode){
    if(item_mode == true){
        std::vector<std::pair<int, int>> vec;
        if(order == 1){
            for(int i=0;i<items.size();i++){
...
void item::del(std::vector<std::string>& items, int index, std::string item, int amount, bool item_mode){
    if(item_mode == true){
        if(index == -1){
