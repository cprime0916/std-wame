...
namespace item {
    ...
    void list(std::string parameter, int way, char custom_key, std::vector<std::string> items,
                    std::vector<bool> starred, bool item_mode) {
        char c = '\n';
        if (way == 0 && item_mode == true) {
            if (parameter.empty() || parameter == "all") {
    ...
    void sort(std::vector<std::string> &items, std::vector<int> &time, std::vector<int> &levels, char sorting_key,
                    bool item_mode) {
        if (item_mode == true) {
    ...
    void star(std::vector<std::string> items, std::vector<bool> &starred, std::string item, bool item_mode) {
        ...
        if (std::count(items.begin(), items.end(), item) > 0) {
            ...
            if (it != items.cend()) {
                ...
                std::cout << "Error 306: Index Not Found Within Inventory" << std::endl;
            }
        } else {
            std::cout << "Error 305: Can't Find Item Within Inventory" << std::endl;
        }
    }

    void sequence(std::vector<std::string> &items, std::vector<bool> is_heal, std::vector<bool> is_harm,
                        std::vector<int> hp, std::vector<int> dmg, bool order, bool item_mode) {
        if (item_mode == true) {
            std::vector<std::pair<int, int>> vec;
            if (order == 1) {
                for (int i = 0; i < items.size(); i++) {
            ...
            std::cout << "Error 304: Unsuccessful Sequencing Operation" << std::endl;
        }
    }

    void del(std::vector<std::string> &items, int index, std::string item, int amount, bool item_mode) {
        if (item_mode == true) {
            if (index == -1) {
                if ((std::count(items.begin(), items.end(), item) > 0) && (amount == -1 || amount == 1)) {
                    std::vector<std::string>::iterator it = std::find(items.begin(), items.end(), item);
                    if (it != items.cend()) {
